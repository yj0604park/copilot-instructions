...
- node heartbeat: `homelab-node-agent.service` (systemd, enabled) → Memo `/nodes` 5분 주기. repo `~/homelab-node-agent`, config `node-agent.json` (hostname=raspberrypi, service_checks 없음)
