...
- secondary DNS: dnsmasq (`dnsmasq.service`, enabled) — 인프라 SPOF 완화용 failover resolver. config `/etc/dnsmasq.d/secondary-dns.conf`. standalone(no-resolv + public upstream 1.1.1.1/8.8.8.8)이라 primary(yozit pihole 10.0.0.172) 다운 시에도 동작. listen: lo/eth0(192.168.50.192)/tailscale0(100.79.4.18) bind-interfaces. 로컬레코드 sync = `scripts/sync-secondary-dns.sh` (yozit SSH 접근 필요, 아직 미구성 → cron 미등록). **네트워크 주의**: rpi(192.168.50.0/24)와 yozit LAN(10.0.0.0/24)이 중첩 NAT라 10.0.0.x 클라이언트는 rpi LAN IP로 직접 도달 불가 → secondary는 tailnet(100.79.4.18) 기준 또는 라우터 포트포워드 필요
